...
export default function SignupPage() {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    const fieldErrs = {};
    if (!form.firstName.trim()) fieldErrs.firstName = 'First name is required.';
    if (!form.lastName.trim()) fieldErrs.lastName = 'Last name is required.';
    if (!form.email.trim()) fieldErrs.email = 'Email is required.';
    if (!form.password) fieldErrs.password = 'Password is required.';
    else if (form.password.length < 8) fieldErrs.password = 'Password must be at least 8 characters.';
    if (form.password && form.confirm !== form.password) fieldErrs.confirm = 'Passwords do not match.';
    ...
    try {
      ...
      else setGlobalError(err.message || 'Signup failed. Please try again.');
    } finally {
      setLoading(false);
    }
  }
        ...
        <div className="glass-card rounded-3xl p-8">
          <form onSubmit={handleSubmit} noValidate>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div>
                <label className="block text-white/60 text-xs font-medium mb-2 uppercase tracking-wider">First name</label>
                <input
                  type="text"
                  ...
                  autoFocus
                  className={`input-glass w-full ${errors.firstName ? 'border-red-500/50' : ''}`}
                />
                {errors.firstName && <p className="mt-1.5 text-red-400 text-xs">{errors.firstName}</p>}
              </div>
              <div>
                <label className="block text-white/60 text-xs font-medium mb-2 uppercase tracking-wider">Last name</label>
                <input
                  type="text"
                  value={form.lastName}
                  ...
                  className={`input-glass w-full ${errors.lastName ? 'border-red-500/50' : ''}`}
                />
                {errors.lastName && <p className="mt-1.5 text-red-400 text-xs">{errors.lastName}</p>}
              </div>
            </div>
            ...
            <div className="mb-6">
              ...
              <input
                ...
                autoComplete="new-password"
                className={`input-glass w-full ${errors.confirm ? 'border-red-500/50' : ''}`}
              />
