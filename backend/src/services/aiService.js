...
Assign real, named attractions and restaurants to each day. Include accurate GPS coordinates for each attraction. Return ONLY valid JSON:
...
3. Tier rules:
   - "full-day" (7–10 hrs): theme parks, major film studios, large safari. ONE attraction only — fills entire day.
   - "half-day" (3–5 hrs): major forts, flagship museums, large palaces. Max 1 standard alongside.
   - "standard" (1.5–2.5 hrs): temples, monuments, smaller museums, markets. Up to 3 per day.
   - "short" (0.5–1 hr): viewpoints, small shrines, snack spots. Supplement only.
4. Day 1: airport arrival + hotel check-in first (tier "short"). Max 1 light standard attraction.
5. Day ${days} (last): light day — check-out and departure. No major sightseeing.
6. Use accurate real GPS coordinates (lat/lng) for every attraction.
7. Generate exactly ${days} days.`;

  const maxTokens = Math.min(days * 220 + 1000, 4096);
...
async function generateFullGuide(groq, { destination, startDate, endDate, budget, interests, days, manifest, topPlacesCount }) {
  ...
  // Stay under Groq free-tier 12,000 TPM limit: prompt ~450+75/day tokens, cap remainder for output
...
Format note: each attraction is Name[tier,durationH,lat,lng]
...
5. Insert a "Transport" activity (30–60 min) between locations >5 km apart.
6. Replace any null meal with "Local eatery near [area]".
7. Copy lat and lng from the locked plan into each activity (0 for transport/meal activities).
...
- keyPhrases (exactly 10 objects: english, local, pronunciation, category)
...
async function generateItinerary({ destination, startDate, endDate, budget, interests }) {
  ...
  try {
    ...
    if (cause?.status === 429) {
      const err = new Error('AI service rate limit reached. Please wait a minute and try again, or try a shorter trip duration.');
      err.status = 429; err.code = 'RATE_LIMIT'; throw err;
    }
    const err = new Error('Failed to generate the trip plan. Please try again.');
    err.status = 500; err.code = 'AI_PLAN_ERROR'; throw err;
  }
  ...
  try {
    ...
    err.status = 500; err.code = 'AI_GUIDE_ERROR'; throw err;
  }
}
